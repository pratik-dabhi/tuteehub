...
class RateLimitTest extends TestCase
{
    ...
    public function test_basic_plan_rate_limit_enforced()
    {
        ...
        $this->withHeader('Authorization',"Bearer $token")->postJson('/api/v1/subscription/subscribe',['plan'=>'Basic']);

        // 3 allowed
        for ($i=0;$i<3;$i++) {
            $r = $this->withHeader('Authorization',"Bearer $token")->getJson('/api/v1/users/'.$user->id);
            $r->assertStatus(200);
        }
